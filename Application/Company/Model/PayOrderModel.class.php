...
class PayOrderModel extends Model{
        ...
        if (!$where) {
            ...
            if ($pageCount>0) {
                ...
                $result=$this->alias('po')
                                ->field('po.*,ci.company_name,sa.name,sa.qq')
                                ->join('left join '.C('DB_PREFIX').'company_info as ci on ci.id=po.company_id')
                                ->join('left join '.C('DB_PREFIX').'user_service_provider as usp on usp.company_id=po.company_id and usp.user_id=po.user_id')
    ...
    public function getPayOrderlistByBillid($billid){
        if ($billid) {
            $result=$this->alias('po')
                    ->field('po.id,po.type')
                    ->where(array('po.service_bill_id'=>intval($billid),'po.state'=>1))
                    ->select();
            if ($result) {
                return $result;
            }else if (null === $result) {
