...
jQuery.validator.addMethod("lgt", function(value, element, rules) {
    var target = $(rules);

    return target.val() - 0 <= value - 0;
}, "输入的值范围有误");

jQuery.validator.addMethod("ltt", function(value, element, rules) {
    var target = $(rules);
...
$.validator.setDefaults({
    ...
    errorPlacement: function(error, element) {
        ...
        if (element.attr('type') == 'radio' ||
            ...
            element.hasClass('hidden-txt') ||
            element[0].tagName == 'SELECT' ||
            element.hasClass('validate-parent')
        ) {
    ...
    messages: {
        password: {
            ...
            unPureLetter: '密码太简单，请重新输入',
            unPureNum: '密码太简单，请重新输入',
            password: '请输入6-20个字符，包含字母、数字或符号'
        }
    }
})
