...
class ModelApi {
    ...
    public static function get_document_model($id = null, $field = null){
        ...
        if(empty($list)){
            ...
            $model = M('Model','onethink_')->where($map)->field(true)->select();
    ...
    public static function get_model_by_id($id){
        return $model = M('Model','onethink_')->getFieldById($id,'title');
    }
    ...
    public static function get_model_attribute($model_id, $group = true){
        ...
        if(!isset($list[$model_id])){
            ...
            $extend = M('Model','onethink_')->getFieldById($model_id,'extend');

            if($extend){
                $map = array('model_id'=> array("in", array($model_id, $extend)));
            }
            $info = M('Attribute','onethink_')->where($map)->select();
            $list[$model_id] = $info;
        ...
        if($group){
            $sort  = M('Model','onethink_')->getFieldById($model_id,'field_sort');
