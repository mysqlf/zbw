 ...
 * @author huajie <banhuajie@163.com>
 */
function get_ser_cate($cate_id = null){
	if($cate_id==''){
		return false;
	}elseif($cate_id==0){
		return '企业介绍';
	}
	$cate   =   M('service_article_category','zbw_')->where('id='.$cate_id)->getField('title');
	return $cate;
}
...
function show_company($id){
   $info = M('company_info')->getFieldByCompany_id($id, 'company_name');
   if($info){
		return $info;
   }else{
		return false;
 ...
 * @param  $[code] [<城市id>]
 */
 function select_location($code = 0){
	if($code){
		if($code%1000000 == 0){
			$_area['pro'] = get_area();
			$_area['pro_check'] = $code;
			return $_area;
		}else{
			$_area['pro'] = get_area();
			//$_area['city_check'] = $code;
			$pre  = intval($code/1000000)*1000000;
			$next = intval($code/1000000)*1000000+1000000;
			$_area['pro_check']  = $pre;
			$_area['code']  = $code;
			$area = getZoning(); 
			foreach ($area as $key => $value) {
					if( $key < $next  && $key >  $pre && $key%10000 == 0 ){
						$_area['city'][$key]['name'] =  $value['name'];
						$_area['city'][$key]['id'] = $key;
					}
			}
			return  $_area;
		}
...
/**
 * [table_prefix 给需要的表添加前缀]
 * @Author   JieJie
 * @DataTime 2016-06-27T16:41:57+0800
...
function table_prefix($table_name)
{
	...
	if(in_array($table_name, $zbw_prefix)) return true;
	return false;
}

/**
 * [getAdminName 通过管理员id获取管理员名称]
 * @Author   JieJie
 * @DataTime 2016-07-07T12:33:47+0800
 * @param    [int]                   $admin_id [管理员id]
 * @return    [string]                             [管理员名称]
 */
function getAdminName($admin_id)
{
	$Model = M('member','onethink_');
	return $Model->where('uid='.intval($admin_id))->getField('nickname');
}

/**
 * [getTemplateNum 获取指定模板id 模板个数]
 * @Author   JieJie
 * @DataTime 2016-07-07T15:25:54+0800
 * @param    [int]                   $template_id [模板id]
 * @return   [int]
 */
function getTemplateNum($template_id)
{
	$condition['template_id'] = intval($template_id);
	$condition['state'] = array('neq',-9);
	$condition['type'] = 1;
	$num = M('TemplateRule','zbw_')->where($condition)->count('template_id');
	return $num ? $num : 0;
}

/**
 * [add_template_log 记录模板修改日志]
 * @Author   JieJie
 * @DataTime 2016-07-08T18:46:05+0800
 * @param    [array]                   $data [如：$data = array('template_id'=>1,'old_rule'=>json,'detail'=>'巴拉巴拉...')]
 */
function add_template_log($data)
{
	if(empty($data) && !is_array($data)) return false;
	$data['admin_id'] = UID;
	$data['create_time'] = date('Y-m-d H:i:s');
	return M('sys_template_log','zbw_')->add($data);
}