...
class ThinkController extends AdminController {
	...
	public function lists($model = null, $p = 0){
		...
		foreach ($grids as &$value) {
			...
			$val      = explode(':', $value);
			// 支持多个字段显示
			$field   =  array($val[0]);//explode(',', $val[0]);
		   if($val[3]){
			  unset($val[2]);
			  $_field = $val[3];
			  $field = array($val[0].'|'.$val[3]);
			}               
		...
		if($model['extend']){
			...
			//JieJie<heihei_yh@163.com> 修改
			$fix    = C("DB_PREFIX");
			table_prefix($parent) && $fix = 'zbw_';
			...
			$prefix = table_prefix($name) ? 'zbw_' : 'onethink_';
			$data = M($name,$prefix)
				/* 查询指定字段，不指定则查询所有字段 */
				->field(empty($fields) ? true : $fields)
				// 查询条件
				...
				->select();
			/* 查询记录总数 */
			$count = M($name,$prefix)->where($map)->count();
		}
	...
	public function edit($model = null, $id = 0){
		...
		if(IS_POST){
			...
			$Model  =   M(parse_name(get_table_name($model['id']),1),$prefix);
			// 获取模型的字段信息
			...
			/*$name = parse_name(get_table_name($model['id']), true);
			$prefix = table_prefix($name) ? 'zbw_' : 'onethink_';*/
			$data       = M(get_table_name($model['id']),$prefix)->find($id);
