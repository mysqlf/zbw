...
class ArticleController extends AdminController {
	...
	/**
	 * [setSerStatus 审核服务商文章]
	 *
	 */
	public function setSerStatus(){

		$id=I('ids');
		$Article=D('ServiceArticle');
		$count=count($id);
		$status=I('request.status');
		if ($status=='x') {#因路由格式原因导致无法正常获取值为-1的状态所以用x代-1参数
			$status=-1;
		}
	...
	public function examine(){
		...
		if($status=='x'){
			...
			$status=array('in',array(0,1,-1));
		}
		
		$Article=D('ServiceArticle');
		
		$where=array('status'=>$status);
		$list=$Article->getArticleBycategory($where);
		$list['data']=self::_actionRelated($list['data']);
		$this->assign('list', $list['data']);
		$this->assign('_page', $list['page']);
		//处理列表数据
		$this->meta_title       =   '服务商文章';
		$this->display();
	}
	/**
	 * [searcharticle 企业文章搜索功能]
	 * @return [type] [description]
	 */
	public function searcharticle(){
		$this->getMenu();
		$companyname=htmlentities(I('param.companyname'));
		$Company=D('CompanyInfo');
		$where['company_name']=array('like',"%$companyname%");
		$companyarr=$Company->getcompanyidbyname($where);
	...
	/**
	 * [_actionRelated 文章相关数据查询]
	 * @param  [type] $data [description]
	 * @return [type]       [description]
	 */
	public function _actionRelated($data){
		$Company=D('CompanyInfo');
		$Category=D('ServiceArticleCategory');
		if ($data) {
			foreach ($data as $key => $value) {
				$data[$key]['username']=$Company->getname($value['company_id']);
				$data[$key]['comname']=$Company->getcompanyname($value['company_id']);
				$data[$key]['catname']=$Category->getcatename($value['category_id']);
			}
		}
		return $data;
	}
	...
	public function del(){
		$result = M('document')->where('id='.intval(I('get.ids',0)))->save(array('status'=>-1));
		if($result) {
			$this->success('操作成功');
		}else{
			$this->error('操作失败!');
